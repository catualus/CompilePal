...
namespace NavPal
{
    ...
    public sealed class BspVisibility
    {
        ...
        ///
        /// **Does not see displacements.** World brushes and brush entities only, where
        /// <see cref="IsLineClear(BspFile.Vector3, BspFile.Vector3, int)"/> also tests displacement
        /// triangles. On terrain this sweep reports open air, so it is not a drop-in replacement for a
        /// line test anywhere the ground itself might be displacement - substituting it into the
        /// standability check took gm_construct's isolated areas from 46 to 321 by accepting samples
        /// under displacement that no body fits in. Safe where the question is about brushwork (a
        /// doorway, a railing, a step between two sampled nodes); not safe as a general clearance test
        /// until displacement sweeping is added.
        /// </summary>
        ...
        public int EnumerateFloors(float x, float y, float topZ, float bottomZ, Span<FloorSample> into)
        {
            ...
            while (count < into.Length && cursor > bottomZ)
            {
                ...
                if (!TryTraceSurface(from, to, GenerationMask, out var point, out var normal))
                    break;
                ...
                while (cursor > bottomZ && IsPointSolid(x, y, cursor, GenerationMask))
                    cursor -= SolidStep;
            }
