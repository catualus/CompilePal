...
namespace NavPal
{
    ...
    public static class AreaGenerator
    {
            ...
            bool squareUp = true, NavProgress? progress = null)
        {
            ...
            var merged = AreaMerger.Merge(nav);
            if (merged.Merges > 0)
        ...
        private static uint NextId(NavFile nav)
        {
            uint next = 1;
            foreach (var area in nav.Areas)
                next = Math.Max(next, area.Id + 1);
        ...
        private sealed class World(BspVisibility vis, BspFile.Vector3 mins, BspFile.Vector3 maxs)
        {
            ...
            private readonly System.Collections.Concurrent.ConcurrentDictionary<(int, int, int), bool> standable = new();
            ...
            /// </summary>
            public bool IsStandable(int gx, int gy, float z)
            {
                var key = (gx, gy, (int)MathF.Round(z / 8f));

                if (standable.TryGetValue(key, out bool cached))
                    return cached;

                float x = mins.X + gx * StepSize;
                float y = mins.Y + gy * StepSize;

                // Still the neighbour-probe test, not the surface normal. Swapping to
                // IsWalkableGround was tried and measured worse on both counts that matter: coverage
                // 89.1% -> 86.7% and runtime 19.7s -> 178s, because TryTraceSurface walks the whole
                // segment across all three geometry classes where IsLineClear stops at the first hit.
                //
                // A narrower version was also tried - fall back to the normal only for cells the probe
                // test already rejects, rather than replacing it everywhere. That measured worse too:
                // one extra area recovered against 14 misses, at the cost of solid-overlap almost
                // doubling (1.1% of areas to 2.0%, one area left 100% buried) because a single point's
                // normal can read as walkable ground while the footprint around it clips a wall the
                // four-probe test would have caught. The neighbour probe belongs alone.
                ...
                //
                // The reason is a gap in the hull trace, not in Valve's rule: TryTraceHull walks the BSP
                // tree and brush entities only, and never consults displacements, where IsLineClear
                // does. On terrain and under displacement ceilings the sweep therefore sees open air and
                // accepts samples no body fits in. Closing that gap is a prerequisite for using the hull
                // anywhere the ground might be displacement - which is most of a Source map.
                bool ok = vis.IsLineClear(
                              new BspFile.Vector3(x, y, z + 2f),
                              new BspFile.Vector3(x, y, z + RequiredHeadroom),
                              BspVisibility.GenerationMask)
                          && IsLevelEnough(x, y, z);

                return standable.GetOrAdd(key, ok);
            }
            ...
            /// </summary>
            private bool IsWalkableGround(float x, float y, float z)
            {
                var from = new BspFile.Vector3(x, y, z + 4f);
                var to = new BspFile.Vector3(x, y, z - 8f);

                return vis.TryTraceSurface(from, to, out _, out var normal)
                       && normal.Z >= NavConstants.SlopeLimit;
            }
